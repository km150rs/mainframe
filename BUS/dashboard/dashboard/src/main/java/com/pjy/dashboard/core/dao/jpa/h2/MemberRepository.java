...
public interface MemberRepository extends JpaRepository<MemberVo, Integer> {
    ...
    @Query(value = "select * from MEMBER where userId = ?1 and companyno=?2", nativeQuery = true)
    MemberVo testserId(String userId,String companyNo);

    @Query(value = "select company_nm from TBL_COMPANYINFO where company_no = ?1", nativeQuery = true)
    String getCompanyNm(String companyNo);
	...
	@Query(value="update MEMBER set userName = :#{#vo.userName},email=:#{#vo.email} WHERE userId = :#{#vo.userId}", nativeQuery=false)
	Integer update(@Param("vo") MemberVo vo );

    @Transactional
	@Modifying	// update , delete Query
	@Query(value="update MEMBER set ldap_mail = :#{#vo.ldap_mail},ldap_displayName=:#{#vo.ldap_displayName},ldap_description=:#{#vo.ldap_description},ldap_mobile=:#{#vo.ldap_mobile} ,last_access_dt = sysdate WHERE userId = :#{#vo.userId}", nativeQuery=false)
	Integer updateByLdap(@Param("vo") MemberVo vo );

    @Transactional
	@Modifying	// update , delete Query
	@Query(value="update MEMBER set password =:#{#vo.password} ,last_access_dt = sysdate WHERE userId = :#{#vo.userId}", nativeQuery=false)
	Integer updatePassword(@Param("vo") MemberVo vo );
