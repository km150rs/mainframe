...
DataTable.render.percentBar = function (pShape, cText, cBorder, cBar, cBack, vRound, bType) {
    ...
    bType = bType || 'ridge';
    //Bar templates
    var styleRule1 = 'max-width:100px;margin-top:-3px;height:10px;margin:0 auto;';
    var styleRule2 = 'border:0px ' +
        bType +
        ' ' +
        cBorder +
        ';line-height:10px;font-size:10px;color:' +
        cText +
        ';background:' +
        cBack +
        ';position:relative;';
    var styleRule3 = 'height:10px;line-height:10px;text-align:center;background-color:' +
        cBar +
        ';padding:auto 6px;';
    ...
    return function (d, type, row) {
        ...
        return ('<div style="' +
            ...
            'width:' +
            s +
            '%;"></div><div style="width:100%;text-align:center;font-size:10px;position:absolute;left:0;top:0;">' +
            value +
            '</div></div></div>');
    };
};
