...
$( function() {
	...
	ChatManager = (function () {
		...
		ChatManager.btnAvg = function () {
			var commonVoStr = ChatManager.setCommonVo(true,'','getEmpWorkingKMRouteAvg','');
						...
						for (var i = 0; i < ret.data.length; i++) {
							...
							for (var k = 0; k < stepDataArray.length; k++) {
								...
								if (stepDataArray[k] == 0)
									 ret.data[i][colNm] = '';
								else ret.data[i][colNm] = parseInt(stepDataArray[k]);

								// 주합계 구한뒤 토요일 하단에 표시
								sumMin += parseInt(stepDataArray[k]);
								if (weekArray[k] == '토') {
									tempData[colNm] = sumMin;
									sumMin = 0;
								}
								// 월 누적 합계와 근무일수 합계
							...
							if (monthCnt > 0) {
								...
								tempData['AVG'] = Math.round(sumMonthMin / monthCnt);
							}
...
function fn_sum_footer (result) {
	...
	var footer = $('#avgTable').append('<tfoot id="pjyFoot"><tr></tr></tfoot>');
	var exp1 = /(avg_km)/;
	...
	for (var i=0;i < result.header.length; i++) {
		...
		if (exp1.test(key)) {
			...
			$(footer).append('<th style="font-size:14px;background:lavender; text-align: center">' + numFormat(total) + '</th>');
		} else if (key == 'emp_cnt') {
	        var total = api.column(i).data().reduce((a, b) => intVal(a) + intVal(b), 0);
			$(footer).append('<th style="font-size:14px;background:lavender; text-align: center">' + numFormat(total) + '</th>');
		} else if (key == 'route_nm') {
			$(footer).append('<th style="background:lavender; text-align: center">전체</th>');
		} else {
			$(footer).append('<th style="background:lavender; text-align: center">-</th>');
		}				
	} 
