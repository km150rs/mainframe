		...
		if (InputOption.optionList[0][colNm] != undefined) {
			...
			if (colNm == 'ACC_DATE') {
				return	'<div  class="tui-datepicker-input tui-datetime-input tui-has-focus">' +
		            	'<input type="text" id="acc_date" aria-label="Date-Time" autocomplete="off">' +
...
*/
	...
	if (getParam("row") == -1) {
	    ...
	    var calMonthKo_from = new tui.DatePicker('#datepicker-from', {
		 ...
		 $('#acc_date').focus();
			saveData['acc_date'] = $('#acc_date').val();
			popupManager.mainGrid.cell(0,1).data($('#acc_date').val()).draw(); 		
		 
		calMonthKo_from.on('change', () => {
			saveData['bit'] = 'Y';
			saveData['acc_date'] = $('#acc_date').val();
			popupManager.mainGrid.cell(0,1).data($('#acc_date').val()).draw(); 		
		});
