 ...
 var dateFormat = "yy-mm-dd",
	...
	editor = new $.fn.dataTable.Editor({
	  ...
	  fields: [
	    ...
	    { label: '근태구분', name: 'attendance_type' 
                ,type:  "select",
                options: [
                    { label: "휴가", value: "휴가" }
                    ,{ label: "병가", value: "병가" }
	...
	editor.on('open', function (e, mode, action) {
		...
		if (action == 'edit') {
			...
			if ( modifier ) {
		        var data = attandanceTable.row( modifier ).data();
	...
	editor.on('preSubmit', function (e, data, action) {
		var modifier = editor.modifier();
		if ( modifier ) {
	        var currData = attandanceTable.row( modifier ).data();
...
/*	$('#attandanceTable tbody').on('click','td', function() {
		var row = attandanceTable.cell(this).index().row;
		var col = attandanceTable.cell(this).index().column;
		var rowData = attandanceTable.cell( row,col ).data();
...
function btnQuery() {
	...
	$.ajax({
	       ...
	       success : function(ret) {	
	    	   console.log(ret);
	    	   if (ret == '[]') return;
	    	   attandanceTable.clear();
	    	   attandanceTable.rows.add( ret).draw( );
	    	  //. swal(ret);
	    	   //tab_1_show();
	       },
	       error : function(request, status, error) {
