...
function render_QueryResult_SP(data,type,row,meta){
	...
	if (meta.col <= 2) {
		...
		iData = iData.replace(AFTER_SET_CHAR,'').replace(OFFDAY_SET_CHAR,'').replace(BEFORE_SET_CHAR,'');
		$(api.cell(meta.row, meta.col).node()).css('background-color', '');	
		return iData;	
	}
	return data;
...
$( function() {
	...
	var ChatManager = (function () {
	    ...
	    *  고정기사별  조회
	    */	    
	    ChatManager.btnSelect = function btnSelect(bPatternShow) {
...
//			$( "#tabs" ).tabs( "option", "active", 0);	// last tab
	    	    ...
	    	    success : function(ret) {	
					...
					ChatManager.sumEmpTotWorkCount(ChatManager.SPTable);
	 	    		...
	 	    		ChatManager.DetailTable.rows( function ( idx, aData, node2 ) {
						// 일별 배정/미배정 합계 산출
						if (idx >2)
							ChatManager.sumDailyAllocateCount(ChatManager.BasicTable,ChatManager.DetailTable,idx);
					});
	    ...
	    *  오전오후별  조회
	    */	    
	    ChatManager.btnSelect2 = function btnSelect2(bPatternShow) {
			$('.pjy_table_fixed').css('visibility', 'visible');
	    	   ...
	    	   "data" : 
	    	    ...
	    	    success : function(ret) {	
					...
					ChatManager.sumEmpTotWorkCount(ChatManager.DetailTable2);
	 	    		...
	 	    		ChatManager.DetailTable2.rows( function ( idx, aData, node2 ) {
						// 일별 배정/미배정 합계 산출
						if (idx >2)
							ChatManager.sumDailyAllocateCount(ChatManager.BasicTable2,ChatManager.DetailTable2,idx);
					});
...
*/		        columns : [
		...
		ChatManager.sumDailyAllocateCount = function (tableA,tableB,col) {
		    ...
		    tableA.cell(4,col).data(String(count));
		    var totCount=tableA.cell(2,col).data();
		    tableA.cell(3,col).data(String(totCount-count));
		}
	...
	var tabs = $( "#tabs" ).tabs(
		{
		  activate: function(e, ui) {
		    ...
		    if (id == 'tabs-0')
		    	ChatManager.btnSelect(false);
		    else ChatManager.btnSelect2(false);
		  }
		}		
	);
