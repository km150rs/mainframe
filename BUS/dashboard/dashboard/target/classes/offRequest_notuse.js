 ...
 var dateFormat = "yy-mm-dd",
	...
	editor = new $.fn.dataTable.Editor({
	  ...
	  fields: [
	    ...
	    { label: '근태구분', name: 'offRequest_type' 
                ,type:  "select",
                options: [
                    { label: "휴가", value: "휴가" }
                    ,{ label: "병가", value: "병가" }
	...
	editor.on('open', function (e, mode, action) {
		...
		if (action == 'edit') {
			...
			if ( modifier ) {
		        var data = offRequestTable.row( modifier ).data();
			...
			//editor.enable( ['emp_nm', 'offRequest_type'] );
		//	editor.show( 'offRequest_type' );			
		}
	});
	...
	editor.on('preSubmit', function (e, data, action) {
		var modifier = editor.modifier();
		if ( modifier ) {
	        var currData = offRequestTable.row( modifier ).data();
...
//console.log(height);
	offRequestTable = $("#offRequestTable").DataTable({
		    "scrollY": height,
	        scrollCollapse: true,
	        dom: 'lBfrtip',
	        scrollX: true,
	        ...
	        columns : [
    	    	...
    	    	{data: "offRequest_type"	,width:'5%'	,editField: "offRequest_type"},
    	        {data: "memo"			,width:'35%'	},
...
/*	$('#offRequestTable tbody').on('click','td', function() {
		var row = offRequestTable.cell(this).index().row;
		var col = offRequestTable.cell(this).index().column;
		var rowData = offRequestTable.cell( row,col ).data();

		editor.edit(row, {
		  title: '근태정보 변경 '+rowData.id,
		  buttons: '확인'
		} );
	});
*/	
...
function btnQuery() {
	...
	commonVo.offRequestType = $('#comboOffRequestType').val(); 	
	commonVo.empNm 			= $('#edtEmpNm').val();
	commonVo.regDate 		= comm_getToday2();
	commonVo.filterType		= 'selectOffRequestInfo';
	var commonVoString = JSON.stringify (commonVo);
	...
	$.ajax({
	       ...
	       success : function(ret) {	
	    	   //console.log(ret);
	    	   if (ret == '[]') return;
	    	   offRequestTable.clear();
	    	   offRequestTable.rows.add( ret).draw( );
	    	  //. swal(ret);
	    	   //tab_1_show();
	       },
	       error : function(request, status, error) {
...
function saveData(data,crudType) {
	...
	if (crudType == 'D') {
		...
		sendVo.regDate 	= data.reg_date;
		url = "/UpdateOffRequestInfo";
	} else {
	    if (data.emp_nm == '' || data.offRequest_type == '' || data.start_date == ''  || data.end_date == '' ) {
		...
		url = "/InsertOffRequestInfo";
	}
	data.delete_yn = 'N';
	sendVo.empNm 		= data.emp_nm;
	sendVo.offRequestType = data.offRequest_type;
