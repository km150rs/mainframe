...
$( function() {
			       ...
			       "dataSrc" : "",
			        ...
			        async: false,
			       	...
			       	success : function(ret) {	
		    	        ...
		    	        fn_setGridData('#newtable',newColumns,initData,12);
	...
	$('#newtable').on('click','td', function() {
		...
		var objCov = $.extend(true, {}, arr[0]); // 객체로	
		var objHeader = $.extend(true, {}, arrHeader); // 객체로	
		var jsonData = JSON.stringify(arrHeader);
		//console.log(jsonData);

		var data = jQuery.param( objCov );
		var header = jQuery.param( objHeader );
		//console.log(data);
		fnMdiOpen('detail','detail_'+row,'/monitor/popupDetail.html?' + data +'&header=' +header, 775,600); // 이게 더 크게	
	});	
