...
$( function() {
	...
	var ChatManager = (function () {
			...
			$.each(arr, function ( idx, item ) {
				...
				table2.rows( function ( idx, aData, node2 ) {
					...
					for (var key in aData){
						value = aData[key];
						if (key > 'col005' && key.startsWith('col')) {
							++cntTotal;
							if (value == '-')	continue;
	    ...
	    ChatManager.btnAnalView = function () {
			...
			var commonVoString = ChatManager.setCommonVo(true,'TBL_ARRANGE_PUNCTUALITY_INFO','TBL_ARRANGE_PUNCTUALITY_INFO_DETAIL','',true);
			//console.log(commonVoString);
			if (commonVoString == null )
				return;
			       ...
			       "dataSrc" : "",
			        ...
			        async: false,
			       	...
			       	success : function(ret) {	
		    	        ...
		    	        fn_setGridDataAnal('#analtable',columns,ret.data,5);
		    	        fn_setGridDataAnal('#newanaltable',newColumns,initData,6);
	    ...
	    ChatManager.btnAnalResult = function () {
			if (!ChatManager.btnAMPMSeq())
				return;
			...
			$.ajax({
			       ...
			       success : function(ret) {	
					  // console.log(ret);
		    	        ...
		    	        gfn_setGridData('#analresulttable',columns,ret.data,height);
	...
	$(document).on('click', '#btnAnalResult', function () {
		ChatManager.btnAnalResult();
	});
...
function fn_setGridDataAnal(targetGrid,gridColumns, gridData,fixedColCount) {/*Function used to get all column names from JSON and bind the html table header*/  
   	$(targetGrid).empty();
